...
void Larger(ListPointer List, ListElementType number, ListPointer *OutList);



int main()
{
   ListPointer List, PredPtr, OutList;
   int pl,i,number,ari8mos;
   ...
   for(i=0; i<pl; i++)
   {
      printf("DWSE ARITHMO GIA EISAGWGH STHN ARXH THS LISTAS: ");
      scanf("%d",&ari8mos);
      fflush(stdin);
      LinkedInsert(&List, ari8mos, PredPtr);
   }
    printf("Arxiki Lista\n");
    LinkedTraverse(List);
    printf("\n");
    ...
    printf("Doste ton ari8mo katofli gia ti dhmiourgia tis neas listas: ");
    scanf("%d",&number);
    Larger(List, number, &OutList);
    printf("Teliki Lista\n");
   ...
   boolean first;
    ...
    while(current!=NULL)
    {
        if (current->Data >= number)
        {
            ...
            LinkedInsert(OutList,current->Data,PrevPtr);
            if(first)
            {
